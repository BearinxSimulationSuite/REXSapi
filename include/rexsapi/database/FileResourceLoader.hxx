...
namespace rexsapi::database
{
  ...
  inline std::vector<std::filesystem::path> TFileResourceLoader::findResources(TLoaderResult& result) const
  {
    ...
    for (const auto& p : std::filesystem::directory_iterator(m_Path)) {
      ...
      if (p.path().extension() == ".xml") {
        if (!std::filesystem::is_regular_file(p.path())) {
          result.addError(TError{fmt::format("Resource '{}' is not a file", p.path().string())});
          continue;
        }
    ...
    if (resources.empty()) {
      result.addError(TError{"No model database files found"});
    }
  ...
  inline std::vector<uint8_t> TFileResourceLoader::load(TLoaderResult& result, const std::filesystem::path& resource)
  {
    ...
    if (!file.good()) {
      result.addError(TError{fmt::format("Resource '{}' cannot be loaded", resource.string())});
