...
TEST_CASE("File utils test")
{
  ...
  {
    auto buffer = rexsapi::loadFile(result, projectDir() / "models" / "rexs-schema.json");
    CHECK(result);
  ...
  {
    auto buffer = rexsapi::loadFile(result, tmpDir.getTempDirectoryPath() / "puschel.txt");
    CHECK_FALSE(result);
    CHECK(buffer.empty());
  }
  ...
  {
    auto buffer = rexsapi::loadFile(result, tmpDir.getTempDirectoryPath());
    CHECK_FALSE(result);
    CHECK(buffer.empty());
  }
  ...
  SUBCASE("Load file with no permission")
  {
    ...
    auto buffer = rexsapi::loadFile(result, path);
    CHECK_FALSE(result);
    CHECK(buffer.empty());
  }
}
